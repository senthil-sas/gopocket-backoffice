<template>
    <div>
        ...
        <form class="gap-10 w-full" @submit.prevent="sendMail()">
            <Listbox as="div" v-model="document" class="max-w-[350px] mb-4">
                <ListboxLabel class="block text-sm font-medium leading-6 text-gray-900">Select document *
                </ListboxLabel>
                <div class="relative mt-2">
                    ...
                    <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
                        ...
                        <ListboxOptions
                            ...
                            <ListboxOption as="template" v-for="i in documents" :key="i.id" :value="i"
                                v-slot="{ active, document }">
                                <li
                                    :class="[active ? 'violet-bg text-white' : 'text-gray-900', 'relative cursor-pointer select-none py-2 pl-8 pr-4']">
                                    <span :class="[document ? 'font-semibold' : 'font-normal', 'block truncate']">{{ i.name
                                    }}</span>
                                    <span v-if="document"
